...
def create_notebook_and_upload(
    ...
    dbx_echo(f"Notebook path: [yellow]{full_path}")
    return full_path
...
def deploy_py_module_as_notebook(
    ...
    dbx_echo(
        f"📝🚀 Creating DLT pipeline notebook from task items/dependencies: {notebook_name}"
    )
...
def handle_deployment_per_file_config_dependency(
    ...
    dbx_echo(
        f"Starting deployment for workflow: {workflow_name} -- Assigned artifact ID: {run_id}"
    )
...
@click.option("--deployment-file", default="deployment.yaml", required=True)
@click.option("--environment", default="DEFAULT", required=True)
@click.option(
    "--as-individual",
    default=False,
    is_flag=True,
...
def deploy_py_dlt2(deployment_file: str, environment: str, as_individual: bool):
    ...
    for workflow in env_config.payload.workflows:
        ...
        handle_deployment_per_file_config_dependency(
            workflow.name, items, deps, workflow, environment, as_individual
        )
