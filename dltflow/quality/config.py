...
class Expectation(pyd.BaseModel):
    ...
    """
    name: str = pyd.Field(
        description="The name of the expectation that is being checked."
    )
...
class CommonWriteOpts(PydanticV2BaseModelLazy):
    ...
    name: t.Optional[str] = pyd.Field(
        ...
        examples=["my_table", "my_view"],
    )
    comment: t.Optional[str] = pyd.Field(
        default=None,
        description="The comment that will be added to the table.",
        examples=["This is a table", "This is a view"],
    )
    ...
    database: t.Optional[str] = pyd.Field(
        default=None, description="The database where the table will be stored."
    )
    tablename: t.Optional[str] = pyd.Field(
        default=None, description="The name of the table that will be created."
    )
...
class AppendFlowConfig(PydanticV2BaseModelLazy):
    ...
    name: t.Optional[str] = pyd.Field(
        default=None, description="The name of the append flow"
    )
    spark_conf: t.Optional[dict] = pyd.Field(
        default=None,
        description="The spark configuration for the execution of the query",
    ...
    comment: t.Optional[str] = pyd.Field(
        default=None, description="The comment that will be added to the table"
    )
...
class ApplyChangesConfig(PydanticV2BaseModelLazy):
    ...
    keys: t.List[str] = pyd.Field(
        ..., description="The keys to join the source and target tables"
    )
    sequence_by: t.Optional[str] = pyd.Field(
        default=None, description="The column to order data by"
    )
    ignore_null_updates: t.Optional[bool] = pyd.Field(
        default=False, description="Flag to ignore null updates"
    )
    apply_as_deletes: t.Optional[bool] = pyd.Field(
        default=None, description="Flag to apply as deletes"
    )
    apply_as_truncates: t.Optional[bool] = pyd.Field(
        default=None, description="Flag to apply as truncates"
    )
    column_list: t.Optional[t.List[str]] = pyd.Field(
        default=None, description="The list of columns to apply changes"
    )
    except_column_list: t.Optional[t.List[str]] = pyd.Field(
        default=None, description="The list of columns to exclude"
    )
    stored_as_scd_type: t.Optional[str] = pyd.Field(
        default=None, description="The type of SCD to store the data as"
    )
    track_history_column_list: t.Optional[t.List[str]] = pyd.Field(
        default=None, description="The fields to exclude from history tracking"
    )
...
class DLTConfig(PydanticV2BaseModelLazy):
    ...
    func_name: str = pyd.Field(
        description="The name of the function that is being wrapped by the DLTMetaMixin"
    )
    kind: str = pyd.Field(
        description="The type of DLT object that will be persisted in the data platform."
    )
    expectations: t.List[Expectation] = pyd.Field(
        description="The expectations that will be checked on the dataset."
    )
    expectation_action: str = pyd.Field(
        description="The action that will be taken on the dataset."
    )
    ...
    def __init__(self, *args, **kwargs):  # pydantic v1 style post init.
        ...
        # Do things after Pydantic validation
        self._expectation_function: t.Callable = _expectation_map.get(
            self.expectation_action
        )
        self._table_or_view_func: t.Callable = (
            dlt.table if self.kind == "table" else dlt.view
        )
        if self.is_streaming_table:
            if (
                sum([self.append_config is not None, self.apply_chg_config is not None])
                != 1
            ):
                raise DLTException(
                    "Only one of append_config or apply_chg_config should be provided"
                )
...
class DLTExecutionConfig(PydanticV2BaseModelLazy):
    ...
    table_or_view_func: t.Callable = pyd.Field(
        ..., description="The table or view function."
    )
