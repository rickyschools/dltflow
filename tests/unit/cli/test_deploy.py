...
from dltflow.cli.deploy import (
    ...
    get_class_name_from_text,
    get_module_info_from_path_name,
    ...
    return ProjectConfig(
        ...
        targets={
            "test": Environment(
                mode="test",
                workspace=Workspace(
                    host="host", root_path="root_path", artifact_path="artifact_path"
                ),
            )
        },
    )
...
def test_get_module_info_from_path_name(tmp_dir):
    ...
    pro_name, pro_path, mod_path = (
        "project_name",
        Path("project_path"),
        "project_path/module.py",
    )
    ...
    with patch("dltflow.cli.deploy.pathlib.Path") as mock_path:
        with patch("dltflow.cli.deploy.get_class_name_from_text") as mock_class:
            ...
            actual_class, actual_mod = get_module_info_from_path_name(
                project_name=pro_name, project_path=pro_path, module_path=mod_path
            )
